...
<div class="space-y-6" x-data="{ showDelete: false, deleteId: null }">
    <div class="flex items-center justify-between">
        <h2 class="text-xl font-bold">{{ $L::t('products.title') }}</h2>
        <div class="flex gap-2">
            <button onclick="document.getElementById('importCsv').click()" class="px-4 py-2 bg-slate-200 dark:bg-slate-700 rounded-lg text-sm font-medium hover:bg-slate-300">
                <i class="fas fa-file-import mr-1"></i> {{ $L::locale() === 'fr' ? 'Importer CSV' : 'Import CSV' }}
            </button>
            ...
            <a href="{{ route('products.create') }}" class="px-4 py-2 bg-primary-600 text-white rounded-lg text-sm font-medium hover:bg-primary-700">
                <i class="fas fa-plus mr-1"></i> {{ $L::t('products.add') }}
            </a>
        </div>
    </div>
    ...
    <form method="GET" class="flex flex-wrap gap-3 bg-white dark:bg-slate-800 rounded-xl p-4 border border-slate-200 dark:border-slate-700">
        ...
        <select name="category" class="border border-slate-300 dark:border-slate-600 rounded-lg px-3 py-2 text-sm bg-transparent">
            <option value="">{{ $L::locale() === 'fr' ? 'Toutes les catégories' : 'All Categories' }}</option>
        ...
        <select name="stock" class="border border-slate-300 dark:border-slate-600 rounded-lg px-3 py-2 text-sm bg-transparent">
            <option value="">{{ $L::locale() === 'fr' ? 'Tout le stock' : 'All Stock' }}</option>
            <option value="low" {{ request('stock') === 'low' ? 'selected' : '' }}>{{ $L::t('nav.low_stock') }}</option>
            <option value="out" {{ request('stock') === 'out' ? 'selected' : '' }}>{{ $L::t('products.out_of_stock') }}</option>
        </select>
    ...
    <div class="bg-white dark:bg-slate-800 rounded-xl shadow-sm border border-slate-200 dark:border-slate-700 overflow-hidden">
        <table class="w-full text-sm">
            <thead class="bg-slate-50 dark:bg-slate-700/30 text-xs text-slate-500 dark:text-slate-400">
                <tr>
                    <th class="px-6 py-3 text-left">{{ $L::t('products.name') }}</th>
                    <th class="px-6 py-3 text-left">{{ $L::t('products.sku') }}</th>
                    <th class="px-6 py-3 text-left">{{ $L::t('products.category') }}</th>
                    <th class="px-6 py-3 text-right">{{ $L::t('products.cost_price') }}</th>
                    <th class="px-6 py-3 text-right">{{ $L::t('products.selling_price') }}</th>
                    <th class="px-6 py-3 text-center">{{ $L::t('products.stock') }}</th>
            ...
            <tbody class="divide-y divide-slate-100 dark:divide-slate-700">
                ...
                <tr class="hover:bg-slate-50 dark:hover:bg-slate-700/20 {{ $product->isLowStock() ? 'bg-amber-50/50 dark:bg-amber-900/10' : '' }}">
                    ...
                    <td class="px-6 py-3 text-right">{{ number_format($product->cost_price, 2) }} {{ $cur }}</td>
                    <td class="px-6 py-3 text-right font-semibold">{{ number_format($product->selling_price, 2) }} {{ $cur }}</td>
                    ...
                    <td class="px-6 py-3 text-center">
                        <span class="inline-flex px-2 py-0.5 text-xs font-medium rounded-full {{ $product->is_active ? 'bg-emerald-100 text-emerald-700' : 'bg-slate-100 text-slate-500' }}">
                            {{ $product->is_active ? ($L::locale() === 'fr' ? 'Actif' : 'Active') : ($L::locale() === 'fr' ? 'Inactif' : 'Inactive') }}
                        </span>
                    </td>
                    <td class="px-6 py-3 text-center">
                ...
                @empty
                <tr><td colspan="8" class="px-6 py-8 text-center text-slate-400">{{ $L::locale() === 'fr' ? 'Aucun produit trouvé' : 'No products found' }}</td></tr>
                @endforelse
            </tbody>
        </table>
    </div>
    ...
    <div x-show="showDelete" x-cloak class="fixed inset-0 z-50 flex items-center justify-center bg-black/50" @click.self="showDelete = false">
        <div class="bg-white dark:bg-slate-800 rounded-xl p-6 max-w-sm w-full text-center" x-transition>
            ...
            <h3 class="text-lg font-bold mb-2">{{ $L::locale() === 'fr' ? 'Supprimer le produit ?' : 'Delete Product?' }}</h3>
            <p class="text-sm text-slate-500 mb-4">{{ $L::locale() === 'fr' ? 'Cette action est irréversible.' : 'This action cannot be undone.' }}</p>
            <div class="flex gap-3 justify-center">
                <button @click="showDelete = false" class="px-4 py-2 bg-slate-200 dark:bg-slate-700 rounded-lg text-sm">{{ $L::t('common.cancel') }}</button>
                <form :action="`{{ url('products') }}/${deleteId}`" method="POST">
                    @csrf @method('DELETE')
                    <button type="submit" class="px-4 py-2 bg-red-600 text-white rounded-lg text-sm font-medium">{{ $L::t('common.delete') }}</button>
                </form>
            </div>
        </div>
    </div>
