...
@section('content')
<div x-data="orderForm()" class="max-w-4xl space-y-6">
    <div class="flex items-center gap-3">
        <a href="{{ route('orders.index') }}" class="text-slate-400 hover:text-slate-600"><i class="fas fa-arrow-left"></i></a>
        <h2 class="text-xl font-bold">Create Online Order</h2>
    </div>

    <form method="POST" action="{{ route('orders.store') }}" @submit.prevent="submitForm($event)">
        @csrf
            ...
            <div class="lg:col-span-2 space-y-6">
                ...
                <div class="bg-white dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700 p-6 space-y-4">
                    ...
                    <div class="relative">
                        ...
                        <input type="text" x-model="productSearch" @input.debounce.300ms="searchProducts()" placeholder="Search products..."
                        ...
                        <div x-show="searchResults.length > 0" class="absolute z-10 w-full mt-1 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-lg shadow-lg max-h-48 overflow-y-auto">
                            <template x-for="p in searchResults" :key="p.id">
                                <button type="button" @click="addItem(p); searchResults = []; productSearch = ''" class="w-full px-4 py-2 text-left text-sm hover:bg-slate-50 dark:hover:bg-slate-700 flex justify-between">
                                    ...
                                    <span class="text-slate-400" x-text="'$' + parseFloat(p.selling_price).toFixed(2) + ' (' + p.stock_quantity + ' in stock)'"></span>
                                </button>
                            </template>
                        </div>
                    </div>
                ...
                <div class="bg-white dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700 p-6">
                    ...
                    <textarea name="notes" rows="3" class="w-full border border-slate-300 dark:border-slate-600 rounded-lg px-3 py-2 text-sm bg-transparent"></textarea>
                </div>
            </div>
            ...
            <div class="space-y-6">
                <div class="bg-white dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700 p-6 space-y-4 sticky top-24">
                    ...
                    <div>
                        ...
                        <select name="payment_method" class="w-full border border-slate-300 dark:border-slate-600 rounded-lg px-3 py-2 text-sm bg-transparent">
                    ...
                    <div>
                        ...
                        <input type="number" name="payment_amount" step="0.01" min="0" class="w-full border border-slate-300 dark:border-slate-600 rounded-lg px-3 py-2 text-sm bg-transparent">
                    </div>

                    <button type="submit" class="w-full py-2.5 bg-primary-600 text-white rounded-lg text-sm font-bold hover:bg-primary-700 transition">
