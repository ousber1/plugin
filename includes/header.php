...
<head>
    ...
    <?php if ($_seo_og_description): ?>
    <meta property="og:description" content="<?= htmlspecialchars($_seo_og_description) ?>">
    <?php endif; ?>
    <?php if ($_seo_og_image): ?>
    <meta property="og:image" content="<?= htmlspecialchars($_seo_og_image) ?>">
    <?php endif; ?>
    ...
    <?php if ($_seo_og_description): ?>
    <meta name="twitter:description" content="<?= htmlspecialchars($_seo_og_description) ?>">
    <?php endif; ?>
    <?php if ($_seo_og_image): ?>
    <meta name="twitter:image" content="<?= htmlspecialchars($_seo_og_image) ?>">
    <?php endif; ?>

    <!-- Webmaster Verification -->
    <?php if ($_seo_gsc): ?>
    <meta name="google-site-verification" content="<?= htmlspecialchars($_seo_gsc) ?>">
    <?php endif; ?>
    <?php if ($_seo_bing): ?>
    <meta name="msvalidate.01" content="<?= htmlspecialchars($_seo_bing) ?>">
    <?php endif; ?>
    <?php if ($_seo_yandex): ?>
    <meta name="yandex-verification" content="<?= htmlspecialchars($_seo_yandex) ?>">
    <?php endif; ?>
    <?php if ($_seo_pinterest): ?>
    <meta name="p:domain_verify" content="<?= htmlspecialchars($_seo_pinterest) ?>">
    <?php endif; ?>
    ...
    })(window,document,'script','dataLayer','<?= htmlspecialchars($_seo_gtm_id) ?>');</script>
    <?php endif; ?>
    ...
    gtag('js', new Date());
    gtag('config', '<?= htmlspecialchars($_seo_ga_id) ?>');
    </script>
    <?php endif; ?>
    ...
    fbq('init', '<?= htmlspecialchars($_px_meta_id) ?>');
    fbq('track', 'PageView');
    </script>
    ...
    ttq._i[e]._u=i,ttq._t=ttq._t||{},ttq._t[e]=+new Date,ttq._o=ttq._o||{},
    ttq._o[e]=n||{};var o=document.createElement("script");o.type="text/javascript",
    o.async=!0,o.src=i+"?sdkid="+e+"&lib="+t;var a=document.getElementsByTagName("script")[0];
    a.parentNode.insertBefore(o,a)};
    ttq.load('<?= htmlspecialchars($_px_tiktok_id) ?>');
    ttq.page();
    }(window,document,'ttq');
    </script>
    <?php endif; ?>
    ...
    snaptr('init', '<?= htmlspecialchars($_px_snap_id) ?>', {});
    ...
    <script type="application/ld+json">
    {
        ...
        "@type": "<?= htmlspecialchars($_schema_type) ?>",
        "name": "<?= htmlspecialchars($_schema_name) ?>",
        <?php if ($_schema_desc): ?>"description": "<?= htmlspecialchars($_schema_desc) ?>",<?php endif; ?>
        <?php if ($_schema_logo): ?>"logo": "<?= htmlspecialchars($_schema_logo) ?>",<?php endif; ?>
        "telephone": "<?= htmlspecialchars($_schema_phone) ?>",
        "url": "<?= htmlspecialchars(APP_URL) ?>",
        <?php if ($_schema_price): ?>"priceRange": "<?= htmlspecialchars($_schema_price) ?>",<?php endif; ?>
        "address": {
            "@type": "PostalAddress",
            "streetAddress": "<?= htmlspecialchars($_schema_address) ?>",
            <?php if ($_schema_city): ?>"addressLocality": "<?= htmlspecialchars($_schema_city) ?>",<?php endif; ?>
            <?php if ($_schema_postal): ?>"postalCode": "<?= htmlspecialchars($_schema_postal) ?>",<?php endif; ?>
            "addressCountry": "<?= htmlspecialchars($_schema_country) ?>"
        }
    }
    </script>
    <?php endif; ?>

    <?php if ($_seo_custom_head): ?>
    <?= $_seo_custom_head ?>
    <?php endif; ?>
